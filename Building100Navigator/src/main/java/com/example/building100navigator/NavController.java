...
public class NavController {
    ...
    Node node9 = new Node("300_400", "the intersection of Hall 300 and Hall 400");
    ...
    List<Node> nodeList = Arrays.asList(node1, node2, node3, node4, node5, node6, node7, node8, node9, node10, node11, node12, node13, node14, node15, node16, node17, node18, node20, node21, node22, node23, node24, node25, node26, node27, node28, node29);
    ...
    void getDirections(ActionEvent event) {
        ...
        //Reset Nodes on each button press, without this we would only be able to point to one node per program run.
        for (Node s : nodeList) {
        ...
        if (startNode == null || endNode == null){
            ...
            for (int i = 0; i < shortestPath.getShortestPathTo(endNode).size(); i++) {
                ...
                if (shortestPath.getShortestPathTo(endNode).size() == 1){
                    directions.append("Warning: Start Node and End Node are the same or an error has occurred\n");
                    directions.append(" go to ");
                    directions.append(shortestPath.getShortestPathTo(endNode).get(i) + "\n");
                } else if (i == 0) {
                    ...
                    user.setDirection();
                    directions.append("Start at " + shortestPath.getShortestPathTo(endNode).get(i) + "\n");
                    ...
                    if (!(shortestPath.getShortestPathTo(endNode).get(i - 1).equals(endNode.getParentHall().getOtherNode(endNode.getClosestNode())))){
                        ...
                        directions.append("Then go to ");

                        directions.append(shortestPath.getShortestPathTo(endNode).get(i) + "\n");
                    }
                    directions.append("Your destination will be found in Hall " + endNode.getParentHall().getParentHall() + "\n");
                    user.setCurrentNode(endNode.getClosestNode());
                    user.setNextNode(endNode.getParentHall().getOtherNode(endNode.getClosestNode()));
                    user.setPreviousDirection(user.getDirection());
                    user.setDirection();
                    directions.append(user.roomDirectionToString(endNode) + "\n");
                    ...
                    directions.append(user.directionChangeToString() + "\n");
                }

            }
            output.setText(directions.toString());
        }

    }

    //Set Help Text on Help Button Press
    @FXML
    void getHelp(ActionEvent event) {
        String helpText = "Acceptable Inputs:\n" + "Room Number, formatted as: Room XXX\n" +
                "\tEx: Room 905\n" +
                "Entrance Names:\n" +
                "\t200 Hall Entrance N\n" +
                "\t200 Hall Entrance S\n" +
